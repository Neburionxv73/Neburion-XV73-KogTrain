...
export function ExerciseRunner({
  ...
  if (finished) {
    ...
    return <div className="panel runner-complete">
      ...
      <div className="completion-insight"><span>Deine Erkenntnis</span><strong>{finalAverage >= 80 ? "Du hast heute sehr sicher gearbeitet." : finalAverage >= 60 ? "Du hast heute eine wertvolle Grundlage weiter aufgebaut." : "Dein Dranbleiben ist heute ein wichtiger Fortschritt."}</strong><p>{finalAverage >= 80 ? "Beim nächsten Training kannst du diese Sicherheit mit einer neuen Variation erweitern." : "Nimm die Strategie mit, die dir heute am meisten Sicherheit gegeben hat."}</p></div><div className="feedback success">Ergebnisse, Trainingszeit und Fortschritt wurden lokal gespeichert.</div>
      <div className="actions"><button className="btn btn-primary" onClick={restart}>Neue Trainingsserie</button><a className="btn btn-secondary" href="/progress">Fortschritt ansehen</a></div>
    </div>;
  }

  return <section className={`panel exercise-runner experience-stage experience-${exercise.domain} type-${exercise.type}`} aria-live="polite">
    ...
    {phase === "study" && memory && "studyItems" in exercise ? <div className="memory-study" role="status">
      ...
      {checked && result && <>
        <div className={`feedback ${result.correct ? "success" : "error"}`}>
          <strong>{result.correct ? "Das hat sicher funktioniert." : result.score > 0 ? `Teilweise richtig: ${result.score}%.` : "Hier entsteht gerade ein neuer Lernschritt."}</strong>
