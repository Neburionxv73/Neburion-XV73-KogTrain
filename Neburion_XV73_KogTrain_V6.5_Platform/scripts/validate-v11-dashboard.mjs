...
expect("V11: progress sections are present", dashboard.includes("Bereiche und Entwicklung") && dashboard.includes("Heute und diese Woche") && dashboard.includes("Trainingsrhythmus"));
...
expect("V11: responsive breakpoints exist", /@media\s*\(max-width:\s*1100px\)/.test(dashboardCss) && /@media\s*\(max-width:\s*(?:760|820)px\)/.test(dashboardCss));
expect("V11: mobile sidebar safely recomposes", dashboardCss.includes(".sidebar") && (dashboardCss.includes("position:relative") || dashboardCss.includes("grid-template-columns:1fr")));
expect("V11: readable light content contrast", dashboardCss.includes("#1D2A32") || dashboardCss.includes("#172333") || dashboardCss.includes("#0F2533"));
expect("V11: teal navigation identity retained", dashboardCss.includes("#087F8C") || dashboardCss.includes("#006D73") || dashboardCss.includes("#00545B"));
