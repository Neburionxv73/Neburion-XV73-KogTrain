...
function makeTask(_id:string, mode:LogicMode, prompt:string, detail:string, correct:string, distractors:string[], explanation:string):LogicTask {
  const options=shuffled([correct,...distractors.filter((item)=>item!==correct)]).slice(0,4);
  return {id:`${mode}:${detail}`,mode,prompt,detail,options,answer:options.indexOf(correct),explanation};
}
...
function analogy(seed:number,difficulty:Difficulty){const variants=[
  ...
  ["Kompass : Richtung = Uhr : ?","Zeit",["Gewicht","Farbe","Temperatur"],"Instrument und angezeigte Größe."],
  ["Saat : Pflanze = Idee : ?","Projekt",["Schatten","Geräusch","Zahl"],"Ausgangspunkt und daraus entstehendes Ergebnis."]
...
function deduction(seed:number,difficulty:Difficulty){
  const easy=[
    ...
    ["Alle Luma sind hell. Reni ist ein Luma.","Reni ist hell",["Reni ist dunkel","Alles Helle ist Luma","Reni ist groß"],"Reni gehört zur beschriebenen Gruppe."]
  ] as const;
  const medium=[
    ...
    ["Alle Seno sind warm. Kein Warmes ist gefroren. Taro ist ein Seno.","Taro ist nicht gefroren",["Taro ist gefroren","Alles Nicht-Gefrorene ist Seno","Taro ist kalt"],"Seno führt zu warm; warm schließt gefroren aus."]
  ] as const;
  const hard=[
    ...
    ["Alle H sind J. Einige H sind K. Kein J ist L.","Einige K sind nicht L",["Alle K sind J","Kein K ist J","Alle L sind H"],"Die H, die K sind, gehören zu J und können daher nicht L sein."]
  ] as const;
...
function operator(seed:number,difficulty:Difficulty){const x=randomInt(2,8),y=randomInt(2,7),variant=seed%4,correct=variant===0?x+y*2:variant===1?x*y+y:variant===2?x*2+y:x+y*3,detail=variant===0?`Wenn a ★ b = a + 2b, was ist ${x} ★ ${y}?`:variant===1?`Wenn a ★ b = a×b + b, was ist ${x} ★ ${y}?`:variant===2?`Wenn a ★ b = 2a + b, was ist ${x} ★ ${y}?`:`Wenn a ★ b = a + 3b, was ist ${x} ★ ${y}?`;return makeTask(`op-${seed}`,"operator","Berechne mit der neuen Operatorregel.",detail,String(correct),[String(correct+1),String(correct-1),String(x*y)],"Setze die beiden Zahlen exakt in die angegebene Operatorregel ein.");}
function exclusion(seed:number,difficulty:Difficulty){const pools=[
  ...
  {values:["10","20","30","45","50"],correct:"45",reason:"Alle anderen sind Vielfache von 10."},
  {values:["14","21","28","35","41"],correct:"41",reason:"Alle anderen sind durch 7 teilbar."}
];const range=difficulty===1?pools.slice(0,3):difficulty===2?pools.slice(2,6):pools.slice(4);const v=range[seed%range.length];return makeTask(`exc-${seed}`,"exclusion","Welche Zahl passt nicht?",v.values.join(" · "),v.correct,v.values.filter(x=>x!==v.correct).slice(0,3),v.reason);}
...
export function createLogicSession(bestScore:number):LogicSession{
  ...
  const candidates=Array.from({length:3},(_,round)=>{const s=seed+round*40;return [sequence(s,difficulty),rule(s+1,difficulty),analogy(s+2,difficulty),deduction(s+3,difficulty),matrix(s+4,difficulty),operator(s+5,difficulty),exclusion(s+6,difficulty),spatial(s+7,difficulty)];}).flat();
  return {difficulty,tasks:finalizeBalancedSessionTasks("logic-v3",candidates,LOGIC_SESSION_LENGTH,96)};
}
