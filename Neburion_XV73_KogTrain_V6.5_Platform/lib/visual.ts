...
function rotation(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`rotation-${seed}-${turns}`, "rotation", `Wie zeigt der Pfeil nach ${turns * 90}° Drehung im Uhrzeigersinn?`, [arrows[start], "↻", `${turns * 90}°`], correct, arrows.filter((item) => item !== correct), "Jede Vierteldrehung entspricht 90 Grad im Uhrzeigersinn.");
}
...
function mirror(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`mirror-${seed}-${useVertical?"v":"h"}`, "mirror", `Welcher Pfeil ist die ${useVertical ? "vertikale" : "horizontale"} Spiegelung?`, [base, useVertical ? "─" : "│", "?"], correct, diagonals.filter((item) => item !== correct), useVertical ? "Bei vertikaler Spiegelung tauschen oben und unten." : "Bei horizontaler Spiegelung tauschen links und rechts.");
}
...
function pattern(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`pattern-${seed}-${picked.join("")}`, "pattern", "Welches Zeichen setzt die visuelle Reihe fort?", [...sequence, "?"], correct, shuffled(shapes.filter((item) => item !== correct)).slice(0, 3), "Die Formen wiederholen sich in einer festen zyklischen Reihenfolge.");
}
...
function matrix(seed: number, difficulty: Difficulty): VisualTask {
  ...
  const visual = difficulty === 1
    ...
    : [a, b, c, b, c, a, c, a, "?"];
  const correct = b;
  return task(`matrix-${seed}-${a}${b}${c}`, "matrix", "Welches Zeichen ergänzt die Matrix?", visual, correct, shuffled(shapes.filter((item) => item !== correct)).slice(0, 3), difficulty === 1 ? "Die beiden Formen wechseln sich regelmäßig ab." : "Jede Zeile verschiebt die Drei-Symbol-Folge um eine Position.");
}
...
function position(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`position-${seed}-${moves.join("")}`, "position", `Folge den Bewegungen ${moves.join(" ")}. Wo landet der Punkt?`, visual, labels[finalIndex], shuffled(labels.filter((_, index) => index !== finalIndex)).slice(0, 3), "Die Bewegungen werden nacheinander im 3×3-Raster ausgeführt.");
}
...
function search(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`search-${seed}-${base}-${target}-${targetIndex}`, "search", `Finde ${target}. An welcher Position steht das Ziel?`, visual, correct, distractors, "Ein einzelner Zielreiz unterscheidet sich von den Distraktoren.");
}
...
function compare(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`compare-${seed}-${variant}`, "compare", "Wie verhalten sich die beiden Formreihen zueinander?", [...left, "|", ...right], correct, ["identisch", "eine Position anders", "umgekehrte Reihenfolge", "nur die Größe ist anders"].filter((item) => item !== correct), "Vergleiche Position, Reihenfolge und Form beider Reihen systematisch.");
}
...
function memory(seed: number, difficulty: Difficulty): VisualTask {
  ...
  return task(`memory-${seed}-${preview.join("")}-${position}`, "memory", `Welches Symbol stand an Position ${position + 1}?`, ["Position", String(position + 1), "?"], correct, shuffled(shapes.filter((item) => item !== correct)).slice(0, 3), "Die Aufgabe prüft den kurzfristigen Abruf einer zuvor gezeigten Symbolfolge.", preview, previewMs);
}
