...
const WORDS_EN=[
 "Apple","Moon","Bridge","Fox","Candle","Cloud","Key","Forest","Window","River","Star","Mountain","Feather","Cup","Leaf","Tower","Rain","Sound","Stone","Path","Lamp","Circle","Mist","Garden",
 "Anchor","Bread","Island","Jacket","Map","Ladder","Mill","Orange","Brush","Spring","Ring","Bowl","Drum","Shore","Bird","Meadow","Branch","Knife","Pillow","Snow","Harbor","Rope","Pear","Comet",
 "Compass","Lantern","Shell","Backpack","Cedar","Cliff","Sail","Tunnel","Crane","Dice","Atlas","Fountain","Pebble","Banner","Forge","Dome","Balcony","Bell","Canoe","Marble","Palm","Rocket","Saddle","Pond","Clock","Vase","Workbench","Tent","Lens","Mosaic","Broom","Cork","Umbrella","Chest","Pyramid","Carton","Aster","Kettle","Torch","Cave",
 "Accent","Stage","Dragon","Oak","Fjord","Globe","Harp","Hedgehog","Jewel","Cactus","Beacon","Coin","Nut","Organ","Parchment","Quartz","Frost","Column","Ink","Cloak","Volcano","Alarm","Yacht","Compass"
];
const SYMBOLS=["◆","●","▲","■","✦","⬟","★","◇","⬢","✚","⬣","◈","○","□","△","⬡","✖","✿","✧","⬥","◐","◒","◉","⬤"];
const normalizeWords=(values:string[])=>values.map(v=>v.toLocaleLowerCase("en-US")).join("|");
...
function wordTask(d:Difficulty,seed:number,words:string[]):MemoryTask{
...
function wordOrderChoiceTask(d:Difficulty,seed:number,words:string[]):MemoryTask{
...
function recognitionTask(d:Difficulty,seed:number,words:string[]):MemoryTask{
...
export function createMemorySession(bestScore:number,language:"de"|"en"="de"):MemorySession{
 const percent=Math.round(bestScore/MEMORY_SESSION_LENGTH*100),difficulty=difficultyFromPercent(percent),seed=createSessionSeed(),words=language==="en"?WORDS_EN:WORDS_DE;
 const rounds=difficulty===3?5:4,candidates=Array.from({length:rounds},(_,round)=>{const o=seed+round*43;return [sequenceTask("digits",difficulty,o+1),digitGapTask(difficulty,o+11),sequenceTask("reverse",difficulty,o+2),wordTask(difficulty,o+3,words),wordOrderChoiceTask(difficulty,o+13,words),symbolTask(difficulty,o+4),symbolPositionTask(difficulty,o+14),positionTask(difficulty,o+5),recognitionTask(difficulty,o+6,words),nbackTask(1,difficulty,o+7),nbackTask(2,difficulty,o+8)]}).flat();
...
export function normalizeMemoryInput(task:MemoryTask,value:string):string{
 ...
 if(task.mode==="words")return value.toLocaleLowerCase("en-US").trim().split(/[\s,;]+/).filter(Boolean).join("|");
 return value.trim();
}
