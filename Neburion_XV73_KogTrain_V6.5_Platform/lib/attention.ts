...
function visualSearch(seed: number, difficulty: Difficulty): AttentionTask {
  ...
  return withAnswer({ id:`search-${seed}-${target}-${targetCount}`, mode:"visual-search", label:"Visuelle Suche", prompt:`Wie oft erscheint ${target}?`, instruction:"Scanne das Feld und zähle nur den Zielreiz.", visual, explanation:`${target} erscheint ${targetCount}-mal im Reizfeld.` }, correct, distractors);
}
...
function ruleSwitch(seed: number): AttentionTask {
  ...
  return withAnswer({ id:`switch-${seed}-${a}-${b}-${shown}`, mode:"rule-switch", label:"Regelwechsel", prompt:useFirstRule ? `Regel A: Reagiere auf ${a}` : `Regel B: Reagiere auf ${b}`, instruction:"Achte auf die aktuelle Regel – nicht auf die Regel der vorherigen Aufgabe.", visual:[shown], explanation:`Für diese Aufgabe galt ${useFirstRule ? "Regel A" : "Regel B"}.` }, correct, [correct==="Reagieren" ? "Ignorieren" : "Reagieren"]);
}
...
function divided(seed: number, difficulty: Difficulty): AttentionTask {
  ...
  return withAnswer({ id:`divided-${seed}-${target}`, mode:"divided", label:"Geteilte Aufmerksamkeit", prompt:`Finde die Kombination ${target}`, instruction:"Beachte gleichzeitig Farbe und Form.", visual, explanation:`Gesucht war genau die Kombination aus ${color} und ${shape}.` }, target, shuffled(set.filter((item)=>item!==target)).slice(0,3));
}
...
function speed(seed: number, difficulty: Difficulty): AttentionTask {
  ...
  return withAnswer({ id:`speed-${seed}-${target}-${shown}`, mode:"speed", label:"Tempo", prompt:`Ist der Reiz ${target}?`, instruction:`Antworte möglichst schnell. Zielzeit: ${difficulty===3?650:difficulty===2?800:950} ms.`, visual:[shown], explanation:shown===target ? "Der Reiz stimmte mit dem Ziel überein." : "Der Reiz war ein anderer." }, shown===target ? "Ja" : "Nein", [shown===target ? "Nein" : "Ja"]);
}
