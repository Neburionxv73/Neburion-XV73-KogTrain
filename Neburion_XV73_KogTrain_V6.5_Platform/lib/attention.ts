...
function visualSearch(seed: number, difficulty: Difficulty): AttentionTask {
  ...
  const targetCount = randomInt(1, difficulty === 3 ? 5 : 3);
  const total = difficulty === 3 ? 20 : difficulty === 2 ? 15 : 10;
  const visual = shuffled([...Array.from({length:targetCount},()=>target), ...Array.from({length:total-targetCount},(_,i)=>distractorPool[(i+seed)%distractorPool.length])]);
  const correct = String(targetCount);
  const distractors = shuffled(["0","1","2","3","4","5","6"].filter((value)=>value!==correct)).slice(0,3);
  return withAnswer({ id:stableId("visual-search", `${target}-${targetCount}-${visual.join("")}`), mode:"visual-search", label:"Visuelle Suche", prompt:`Wie oft erscheint ${target}?`, instruction:"Scanne das Feld und zähle nur den Zielreiz.", visual, explanation:`${target} erscheint ${targetCount}-mal im Reizfeld.` }, correct, distractors);
}

function ruleSwitch(seed: number): AttentionTask {
  const [a,b] = shuffled(symbols).slice(0,2);
  const useFirstRule = seed % 2 === 0;
  const shown = seed % 4 < 2 ? a : b;
  const correct = useFirstRule ? (shown===a ? "Reagieren" : "Ignorieren") : (shown===b ? "Reagieren" : "Ignorieren");
  return withAnswer({ id:stableId("rule-switch", `${useFirstRule?"A":"B"}-${a}-${b}-${shown}`), mode:"rule-switch", label:"Regelwechsel", prompt:useFirstRule ? `Regel A: Reagiere auf ${a}` : `Regel B: Reagiere auf ${b}`, instruction:"Achte auf die aktuelle Regel – nicht auf die Regel der vorherigen Aufgabe.", visual:[shown], explanation:`Für diese Aufgabe galt ${useFirstRule ? "Regel A" : "Regel B"}.` }, correct, [correct==="Reagieren" ? "Ignorieren" : "Reagieren"]);
}
...
function inhibition(seed: number): AttentionTask {
  ...
  return withAnswer({ id:stableId("inhibition", `${filled}-${showFilled?"filled":"outline"}`), mode:"inhibition", label:"Reaktionshemmung", prompt:`Reagiere nur auf die gefüllte Form ${filled}`, instruction:"Ähnliche Konturen sind Distraktoren.", visual:[showFilled ? filled : outline], explanation:showFilled ? "Die Form war gefüllt: reagieren." : "Die Kontur war ungefüllt: Reaktion hemmen." }, showFilled ? "Reagieren" : "Ignorieren", [showFilled ? "Ignorieren" : "Reagieren"]);
}

function divided(seed: number, difficulty: Difficulty): AttentionTask {
  const set = difficulty === 1 ? ["🔴▲","🔵■","🟢●","🟡◆"] : ["🔴▲","🔴■","🔵▲","🔵■","🟢●","🟢◆","🟡●","🟡◆"];
  const target = shuffled(set)[0];
  const visual = shuffled(set).slice(0, difficulty === 3 ? 7 : 4);
  if (!visual.includes(target)) visual[0] = target;
  return withAnswer({ id:stableId("divided", `${target}-${visual.join("")}`), mode:"divided", label:"Geteilte Aufmerksamkeit", prompt:`Finde die Kombination ${target}`, instruction:"Beachte gleichzeitig Farbe und Form.", visual, explanation:`Gesucht war genau die Kombination ${target}.` }, target, shuffled(set.filter((item)=>item!==target)).slice(0,3));
}
...
function speed(seed: number, difficulty: Difficulty): AttentionTask {
  ...
  return withAnswer({ id:stableId("speed", `${target}-${shown}`), mode:"speed", label:"Tempo", prompt:`Ist der Reiz ${target}?`, instruction:`Antworte möglichst schnell. Zielzeit: ${difficulty===3?650:difficulty===2?800:950} ms.`, visual:[shown], explanation:shown===target ? "Der Reiz stimmte mit dem Ziel überein." : "Der Reiz war ein anderer." }, shown===target ? "Ja" : "Nein", [shown===target ? "Nein" : "Ja"]);
}
...
export function createAttentionSession(bestAccuracy: number): AttentionSession {
  ...
  const candidates = Array.from({length:3},(_,round)=>{
    const s=seed+round*30;
    return [goNoGo(s+1),visualSearch(s+2,difficulty),ruleSwitch(s+3),inhibition(s+4),divided(s+5,difficulty),speed(s+6,difficulty),interference(s+7)];
  }).flat();
  const tasks = finalizeBalancedSessionTasks("attention-v3", candidates, 8, 84);
