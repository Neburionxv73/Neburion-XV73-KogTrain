...
function alternatingRotation(seed:number,difficulty:Difficulty):VisualTask {
  ...
  return visualTask(
    `v6-rotation-alt-${start}-${first}-${second}`,
    "rotation",
    "Welche Richtung entsteht nach zwei aufeinanderfolgenden Drehungen?",
    [ARROWS[start],first===1?"↻":"↺","90°",second===1?"↻":"↺",`${second*90}°`,"?"],
    ARROWS[final],
    ARROWS.filter((item)=>item!==ARROWS[final]),
    "Beide Drehungen werden nacheinander auf dieselbe Ausgangsrichtung angewendet.",
  );
}
...
function mirrorThenRotate(seed:number,difficulty:Difficulty):VisualTask {
  ...
  return visualTask(
    `v6-mirror-rotate-${base}-${turns}`,
    "mirror",
    `Welche Form entsteht nach Spiegelung und anschließender ${turns*90}°-Drehung?`,
    [base,"│","→",mirrored,"↻",`${turns*90}°","→","?"],
    correct,
    DIAGONALS.filter((item)=>item!==correct),
    "Zuerst wird gespiegelt, danach das bereits gespiegelte Ergebnis gedreht.",
  );
}
...
function diagonalRelation(seed:number,difficulty:Difficulty):VisualTask {
  const starts=[0,2,6,8,4];
  const start=starts[seed%starts.length];
  const row=Math.floor(start/3),col=start%3;
  const moves=[
    {label:"diagonal rechts unten",dr:1,dc:1},
    {label:"diagonal links unten",dr:1,dc:-1},
    {label:"diagonal rechts oben",dr:-1,dc:1},
    {label:"diagonal links oben",dr:-1,dc:-1},
  ].filter((move)=>row+move.dr>=0&&row+move.dr<3&&col+move.dc>=0&&col+move.dc<3);
  const move=moves[(seed+difficulty)%moves.length];
  const target=(row+move.dr)*3+(col+move.dc);
  const visual=Array.from({length:9},(_,index)=>index===start?"●":"·");
  return visualTask(
    `v6-position-diagonal-${start}-${target}`,
    "position",
    `Welches Feld liegt ${move.label} vom markierten Punkt?`,
    visual,
    POSITION_LABELS[target],
    shuffled(POSITION_LABELS.filter((_,index)=>index!==target)).slice(0,3),
    "Die gesuchte Position liegt diagonal zum markierten Ausgangsfeld.",
  );
}

function visualAlternation(seed:number,difficulty:Difficulty):VisualTask {
  const [a,b,c]=shuffled(SHAPES).slice(0,3);
  const visual=difficulty===1?[a,b,a,b,a,"?"]:
    difficulty===2?[a,b,c,a,b,c,a,"?"]:[a,b,a,c,a,b,a,c,a,"?"];
  const correct=difficulty===1?b:difficulty===2?b:b;
  return visualTask(
    `v6-pattern-alt-${difficulty}-${a}${b}${c}`,
    "pattern",
    "Welches Zeichen setzt das Wechselmuster fort?",
    visual,
    correct,
    shuffled(SHAPES.filter((item)=>item!==correct)).slice(0,3),
    difficulty===3?"Zwei verschachtelte Wechselmuster laufen parallel.":"Die sichtbaren Formen folgen einem wiederkehrenden Wechselmuster.",
  );
}
...
function matrixShift(seed:number,difficulty:Difficulty):VisualTask {
  ...
  if(difficulty===1){
    return visualTask(`v6-matrix-pair-${a}${b}`,"matrix","Welches Zeichen ergänzt die Matrix?",[a,b,b,a,a,b,b,"?"],a,shuffled(SHAPES.filter((x)=>x!==a)).slice(0,3),"Jede Zweiergruppe tauscht ihre Reihenfolge.");
  }
  if(difficulty===2){
    return visualTask(`v6-matrix-shift-${a}${b}${c}`,"matrix","Welche Form fehlt in der verschobenen Matrix?",[a,b,c,b,c,a,c,a,"?"],b,shuffled(SHAPES.filter((x)=>x!==b)).slice(0,3),"Jede Zeile verschiebt die Dreierfolge um eine Position.");
  }
...
function denseSearch(seed:number,difficulty:Difficulty):VisualTask {
  const [target,base]=shuffled(SHAPES).slice(0,2);
  const total=difficulty===1?8:difficulty===2?14:20;
  const targetCount=difficulty===1?1:difficulty===2?2:3;
  const visual=shuffled([...Array.from({length:targetCount},()=>target),...Array.from({length:total-targetCount},()=>base)]);
  return visualTask(
    `v6-search-count-${target}-${base}-${total}-${targetCount}`,
    "search",
    `Wie oft erscheint der Zielreiz ${target}?`,
    visual,
    String(targetCount),
    shuffled(["0","1","2","3","4"].filter((item)=>item!==String(targetCount))).slice(0,3),
    "Nur der exakte Zielreiz zählt; ähnliche Wiederholungen dienen als Ablenkung.",
  );
}

function compareNearMiss(seed:number,difficulty:Difficulty):VisualTask {
  const length=difficulty===1?4:difficulty===2?5:6;
  const left=shuffled(SHAPES).slice(0,length);
  const right=[...left];
  const variant=seed%3;
  let correct="identisch";
  if(variant===1){
    const i=seed%length,j=(i+1)%length;
  ...
  return visualTask(
    `v6-compare-near-${left.join("")}-${variant}`,
    "compare",
    "Welche Beziehung besteht zwischen den beiden Reihen?",
    [...left,"|",...right],
    correct,
    ["identisch","eine Position anders","umgekehrte Reihenfolge","nur die Größe ist anders"].filter((item)=>item!==correct),
    "Vergleiche Reihenfolge und Positionen besonders genau; die Abweichung kann sehr klein sein.",
  );
}

function visualMemoryPair(seed:number,difficulty:Difficulty):VisualTask {
  const length=difficulty===1?4:difficulty===2?6:8;
  const preview=shuffled(SHAPES).slice(0,length);
  const first=seed%length;
  const second=(first+(difficulty===1?1:2))%length;
...
export function createVisualSession(
  ...
  for (let round = 0; round < 8; round += 1) {
    ...
    const seed = round * 23 + completedSessions * 11 + bestScore;
    candidates.push(
      counterRotation(seed,difficulty),
      alternatingRotation(seed+3,difficulty),
      doubleMirror(seed+5,difficulty),
      mirrorThenRotate(seed+7,difficulty),
      positionRelation(seed+9,difficulty),
      diagonalRelation(seed+11,difficulty),
      visualAlternation(seed+13,difficulty),
      matrixShift(seed+15,difficulty),
      denseSearch(seed+17,difficulty),
      compareNearMiss(seed+19,difficulty),
      visualMemoryPair(seed+21,difficulty),
    );
  }
