...
const answers: Record<string,string> = {
  ...
  "Welche Richtung entsteht nach zwei aufeinanderfolgenden Drehungen?":"Which direction results after two consecutive rotations?",
  "Beide Drehungen werden nacheinander auf dieselbe Ausgangsrichtung angewendet.":"Both rotations are applied consecutively to the same starting direction.",
  "Zuerst wird gespiegelt, danach das bereits gespiegelte Ergebnis gedreht.":"First mirror the shape, then rotate the already mirrored result.",
  "Die gesuchte Position liegt diagonal zum markierten Ausgangsfeld.":"The target position lies diagonally from the marked starting cell.",
  "Welches Zeichen setzt das Wechselmuster fort?":"Which symbol continues the alternating pattern?",
  "Die sichtbaren Formen folgen einem wiederkehrenden Wechselmuster.":"The visible shapes follow a recurring alternating pattern.",
  "Zwei verschachtelte Wechselmuster laufen parallel.":"Two nested alternating patterns run in parallel.",
  "Jede Zweiergruppe tauscht ihre Reihenfolge.":"Each pair swaps its order.",
  "Welche Form fehlt in der verschobenen Matrix?":"Which shape is missing from the shifted matrix?",
  "Welche Form schließt die Vierer-Matrix?":"Which shape completes the four-shape matrix?",
  "Vier Formen rotieren zyklisch über die Zeilen.":"Four shapes rotate cyclically across the rows.",
  "Nur der exakte Zielreiz zählt; ähnliche Wiederholungen dienen als Ablenkung.":"Only the exact target stimulus counts; similar repetitions are distractors.",
  "Welche Beziehung besteht zwischen den beiden Reihen?":"What relationship exists between the two sequences?",
  "Vergleiche Reihenfolge und Positionen besonders genau; die Abweichung kann sehr klein sein.":"Compare order and positions carefully; the difference may be very small.",
  "Zwei Positionen aus derselben zuvor gezeigten Symbolfolge müssen gemeinsam erinnert werden.":"Two positions from the same previously shown symbol sequence must be recalled together.",
};
...
const rules:Array<[RegExp,Replacement]> = [
  ...
  [/^Welche Form entsteht nach Spiegelung und anschließender (\d+)°-Drehung\?$/g,"Which shape results after mirroring and then a $1° rotation?"],
  ...
  [/^Welches Feld liegt diagonal rechts unten vom markierten Punkt\?$/g,"Which cell is diagonally down-right from the marked point?"],
  [/^Welches Feld liegt diagonal links unten vom markierten Punkt\?$/g,"Which cell is diagonally down-left from the marked point?"],
  [/^Welches Feld liegt diagonal rechts oben vom markierten Punkt\?$/g,"Which cell is diagonally up-right from the marked point?"],
  [/^Welches Feld liegt diagonal links oben vom markierten Punkt\?$/g,"Which cell is diagonally up-left from the marked point?"],
  ...
  [/Zwei Spiegelungen werden nacheinander ausgeführt; die zweite arbeitet mit dem bereits gespiegelten Ergebnis\./g,"Two reflections are applied in sequence; the second uses the already mirrored result."],
  ...
  [/^Wie oft erscheint der Zielreiz (.+)\?$/g,"How often does the target stimulus $1 appear?"],
